...
def run_npt(options):
    ...
    ##run first 500 steps (1ps)
    for i in range(100):
        simulation.step(500)
        state = simulation.context.getState(getPositions=True,enforcePeriodicBox=True, getEnergy=True)
        crds_A = state.getPositions(asNumpy=True).value_in_unit(angstrom)
        box_A = state.getPeriodicBoxVectors(asNumpy=True).value_in_unit(angstrom)

        step_PE = state.getPotentialEnergy()
        last_PE.append(step_PE)
        step_V = state.getPeriodicBoxVolume()
        last_V.append(step_V)
    last_PE.pop(0)
    last_V.pop(0)

    ##record
    if 'save_pdb' in options:
        fname = options['save_pdb']
        fname_pdb_out = fname
        write_pdb(options['pdb'], fname_pdb_out, crds_A, box_A)
    if 'save_state' in options:
        fname = options['save_state']
        fname_rst_out = fname
        simulation.saveState(fname_rst_out)

    ##add convergence condition
    def converged_PE(arr):
        std = np.std(arr)
        if (std <= 1500):
            return True
        else:
    ...
    PE_conv = converged_PE(last_PE)
    V_conv = converged_V(last_V)


    while (PE_conv == False and V_conv == False):
        simulation.step(500)
