...
def run_nvt(options):
    ...
    ##restraints correspond to first run 
    if "fc_pos" in options:
        # Restraints
        fc_pos = options['fc_pos']  # kJ/mol/nm^2
        prt_rest = CustomExternalForce('fc_pos*periodicdistance(x,y,z,x0,y0,z0)^2')
    ...
    ##restrains correspond to second run 
    if "fc_prt" in options:
    # Restraints
        fc_prt = options['fc_prt']  # kJ/mol/nm^2
        prt_rest = CustomExternalForce('fc_prt*periodicdistance(x,y,z,x0,y0,z0)^2')
        
        prt_rest.addGlobalParameter('fc_prt', fc_prt)
        prt_rest.addPerParticleParameter('x0')
        prt_rest.addPerParticleParameter('y0')
        prt_rest.addPerParticleParameter('z0')
